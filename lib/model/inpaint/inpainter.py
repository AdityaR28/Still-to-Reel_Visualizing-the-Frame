...
class Inpainter():
    ...
    def process_depth_for_network(depth, context, log_depth=True):
        if log_depth:
            log_depth_val = torch.log(depth + 1e-8) * context
            mean_depth = torch.mean(log_depth_val[context > 0])
            zero_mean_depth = (log_depth_val - mean_depth) * context
        else:
    ...
    def inpaint_depth(self, depth, holes, context, edge, depth_range):
        ...
        inpainted_depth[context > 0.5] = depth[context > 0.5]
        # If the inpainted depth in the background is smaller than the foreground depth,
